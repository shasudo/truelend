...
import { schedulePartnerBackgroundTask } from "@/lib/owned-request-context";
...
async function deleteObject(
  ...
  try {
    ...
    reportPartnerActionFailure(event, error);
  }
}
...
export async function POST(req: Request) {
  try {
    return await withPartnerRequest(req.headers, async ({ session, partner, db, ctx, env }) => {
      if (!session) {
        return Response.json({ error: "Please sign in again." }, { status: 401 });
      }
      if (!partner) {
        return Response.json(
          { error: "Your Referral Partner profile could not be found." },
          { status: 403 },
        );
      }
      ...
      try {
        ...
        if (
          !(await env.PARTNER_WRITE_RATE_LIMITER.limit({ key: `kyc:${partner.userId}` })).success
        ) {
          return Response.json(
        ...
        if (outcome.status !== 200) {
          await deleteObject(env.BUCKET, uploadedKey, "kyc_upload_rollback_delete_failed");
        ...
        if (outcome.superseded.length > 0) {
          schedulePartnerBackgroundTask(ctx, "kyc_superseded_delete_task_failed", () =>
            Promise.all(
              outcome.superseded.map((key) =>
                deleteObject(env.BUCKET, key, "kyc_superseded_delete_failed"),
              ),
            ),
          );
        }
        ...
        // Once COMMIT has been attempted, a rejected client promise does not
        // prove rollback. Preserve the private object so a possibly committed
        // database row can never be left pointing at deleted bytes. Orphans can
        // be reconciled safely; a dangling KYC row cannot.
        if (uploadedKey && !databaseOutcomeUnknown) {
          await deleteObject(env.BUCKET, uploadedKey, "kyc_upload_cleanup_failed");
        }
        reportPartnerActionFailure("kyc_upload_failed", error);
        return Response.json(
          {
            error: databaseOutcomeUnknown
              ? "We couldn't confirm this upload. Refresh the document list before trying again."
    ...
    reportPartnerActionFailure("kyc_upload_context_failed", error);
