...
void test("auth route: a rate-limited sensitive path returns 429 without constructing better-auth", async () => {
  setFakeAuthState({ env: buildEnv({ rateLimited: true }) });

  const response = await GET(new Request("https://admin.example.com/api/auth/sign-in/email"));
...
void test("auth route: an allowed request delegates to better-auth's handler verbatim", async () => {
  ...
  const response = await GET(new Request("https://admin.example.com/api/auth/get-session"));
...
void test("auth route: a handler failure still schedules cleanup before propagating", async () => {
  ...
  await assert.rejects(
    () => GET(new Request("https://admin.example.com/api/auth/get-session")),
